...
object RestaurantAPISpec extends SpecificationWithJUnit with Mockito {
  ...
  "Restaurant api" should {
    ...
    "return no content" in {
      val restaurantAPI = initializeRestaurantAPIGet(None)

      status(restaurantAPI.get(Owner, "0")(FakeRequest())) must equalTo(NO_CONTENT)
    }

    s"$Restaurant1" in {
      val restaurantAPI = initializeRestaurantAPIGet(Some(Restaurant1))

      val result = restaurantAPI.get(Owner, "0")(FakeRequest())

      status(result) must equalTo(OK)
      contentType(result) must beSome("application/json")
      charset(result) must beSome("utf-8")
      contentAsJson(result).as[Restaurant] must equalTo(Restaurant1)
    }
    ...
    "fail to update restaurant with validation error" in {
      val restaurantAPI = initializeRestaurantAPIUpdateFail()

      val request: Request[JsObject] = FakeRequest().withBody(Json.obj("owner" -> Owner))
      val result = restaurantAPI.update(Owner, "0")(request)
    ...
    "fail to update restaurant with persistence error" in {
      val restaurantAPI = initializeRestaurantAPIUpdateFail()
      ...
      val message = contentAsJson(result)
      (message \ "status").as[String] must equalTo("failure")
      (message \ "code").as[Int] must equalTo(RESTAURANT_UPDATE_FAILED.code)
      (message \ "message").as[String] must equalTo(RESTAURANT_UPDATE_FAILED.messageKey)
      (message \ "reason").asOpt[JsObject].isDefined must beFalse
    }

    "success to update restaurant" in {
      val restaurantAPI = initializeRestaurantAPIUpdateSuccess()
  ...
  private def initializeRestaurantAPIGet(response: Option[Restaurant]): RestaurantAPI = {
    val queryBuilder = mock[GenericQueryBuilder[JsObject, Reads, Writes]]

    val collection = mock[JSONCollection]
    collection.find(any[JsObject])(any[Writes[JsObject]]) returns queryBuilder

    queryBuilder.one[Restaurant] returns Future {
      response
    }
  ...
  private def initializeRestaurantAPIUpdateFail(): RestaurantAPI = {
    val collection = mock[JSONCollection]
    collection.update[JsObject, Restaurant](any[JsObject], any[Restaurant], any[GetLastError], any[Boolean], any[Boolean])(any[Writes[JsObject]], any[Writes[Restaurant]], any[ExecutionContext]) returns Future {
      LastError(
        ok = false,
        err = Some("error"),
        code = Some(0),
  ...
  private def initializeRestaurantAPIUpdateSuccess(): RestaurantAPI = {
    val collection = mock[JSONCollection]
    collection.update[JsObject, Restaurant](any[JsObject], any[Restaurant], any[GetLastError], any[Boolean], any[Boolean])(any[Writes[JsObject]], any[Writes[Restaurant]], any[ExecutionContext]) returns Future {
      LastError(
        ok = true,
